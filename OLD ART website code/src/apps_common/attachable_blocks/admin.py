...
class BaseAttachedBlocksMixinChecks(GenericInlineModelAdminChecks):
    def check(self, cls, **kwargs):
        errors = super().check(cls, **kwargs)
...
class BaseAttachedBlocksMixin(ModelAdminInlineMixin):
    ...
    def get_formset(self, request, obj=None, **kwargs):
        ...
        FormSet.can_add = int(request.user.has_perm('attachable_blocks.add_attachablereference'))
        FormSet.can_change = int(request.user.has_perm('attachable_blocks.change_attachablereference'))
        FormSet.can_delete = int(request.user.has_perm('attachable_blocks.delete_attachablereference'))
        return FormSet
...
class AttachedBlocksTabularInline(BaseAttachedBlocksMixin, SortableGenericTabularInline):
    ...
    sortable = 'sort_order'
    template = 'attachable_blocks/tabular.html'
...
class AttachedBlocksStackedInline(BaseAttachedBlocksMixin, SortableGenericStackedInline):
    ...
    checks_class = BaseAttachedBlocksMixinChecks
    sortable = 'sort_order'
    template = 'attachable_blocks/stacked.html'
