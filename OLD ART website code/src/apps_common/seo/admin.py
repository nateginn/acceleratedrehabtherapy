...
class SeoModelAdminMixin(ModelAdminMixin):
    ...
    def save_seo_form(self, request, obj, change=False):
        ...
        if seo_form.is_valid() and seo_form.has_changed():
            ...
            if is_add:
                seo_model_admin.log_addition(request, seo_instance, change_message)
            else:
...
class DummyRobotsAdmin(ModelAdminMixin, admin.ModelAdmin):
    ...
    def get_urls(self):
        ...
        urls = [
            ...
            url(r'^(.+)/change/$', RedirectView.as_view(url=reverse_lazy('admin:%s_%s_add' % info)), name='%s_%s_change' % info),
        ]
        return urls
