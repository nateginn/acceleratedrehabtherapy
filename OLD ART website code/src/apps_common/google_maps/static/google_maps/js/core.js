(function($) {
    ...
    window.GMapMarker = Class(window.GMapObject, function GMapMarker(cls, superclass) {
        ...
        cls.makeNative = function() {
            ...
            try {
                var position = this.options.position || { lat: 40.7128, lng: -74.0060 };
                var latLng = new google.maps.LatLng(
                    parseFloat(position.lat) || 40.7128,
                    parseFloat(position.lng) || -74.0060
                );

                this.native = new google.maps.Marker({
                    position: latLng,
                    map: this.map ? this.map.native : null,
                    title: this.options.title || '',
                    icon: this.options.icon || null,
                    draggable: Boolean(this.options.draggable)
                });

                this.trigger('ready', [this.native]);
            } catch (error) {
    ...
    window.GMap = Class(window.GMapEventedObject, function GMap(cls, superclass) {
        ...
        cls = {
            ...
            makeNative: function() {
                ...
                var defaultOptions = {
                    zoom: 14,
                    center: new google.maps.LatLng(40.7128, -74.0060), // Default to New York
                    styles: [
                        {
                            "featureType": "all",
                ...
                try {
                    this.native = new google.maps.Map(mapElement, options);
                    this.trigger('ready', [this.native]);
                } catch (error) {
            ...
            center: function(value) {
                if (value === undefined) {
                    return this.native ? this.native.getCenter() : null;
                }
                
                if (this.native && value) {
                    try {
                        var center = (value instanceof google.maps.LatLng) 
                            ? value 
    ...
    if (!window.googleMapsInitialized) {
        ...
        script.src = 'https://maps.googleapis.com/maps/api/js?key=' + 
                    ...
                    '&callback=init_google_maps&language=' + 
