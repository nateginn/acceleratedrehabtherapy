...
class Command(BaseCommand):
    ...
    def autopost_twitter(self):
        posts = FeedPost.objects.scheduled_for(conf.NETWORK_TWITTER)[:MAX_POSTS_PER_CALL]
        for post in posts:
            try:
    ...
    def autopost_facebook(self):
        posts = FeedPost.objects.scheduled_for(conf.NETWORK_FACEBOOK)[:MAX_POSTS_PER_CALL]
        for post in posts:
            try:
    ...
    def autopost_linkedin(self):
        posts = FeedPost.objects.scheduled_for(conf.NETWORK_LINKEDIN)[:MAX_POSTS_PER_CALL]
        for post in posts:
            try:
